...
def fit_one_epoch(model_train, model, focal_loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda):
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            _, regression, classification, anchors, landmarks = model_train(images)

            loss_value, _, _ = focal_loss(classification, regression, anchors,landmarks, targets, cuda=cuda)
