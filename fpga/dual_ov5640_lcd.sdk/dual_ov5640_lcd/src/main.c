...
VideoMode vd_mode;
XOv5640_rgb2gray rgb2gray_inst; //PL 端 XOv5640_rgb2gray 驱动实例
unsigned int lcd_id;
...
int main(void) {
	...
	//配置灰度转换 IP 核 OV5640_RGB2GRAY 的列数
	 XOv5640_rgb2gray_Set_cols(&rgb2gray_inst, vd_mode.width/2);
	...
	*bf_size = *bmp_size + 54;
	...
	while (1) {
		xil_printf("input begin",pic_cnt);
		//testUart();


		u32 input = 0;
		input=getInputTel();
		xil_printf("input is %d\r\n",input);
		sleep(1);
		/*sleep(5);
		xil_printf("capture picture\n");
			//获取当前读通道操作的帧缓存编号
			rd_index = XAxiVdma_CurrFrameStore(&vdma[1], XAXIVDMA_READ);
			xil_printf("current read frame is %d\n", rd_index);
			//读通道驻停在当前帧
			XAxiVdma_StartParking(&vdma[1], rd_index, XAXIVDMA_READ);
			//并获取当前帧的起始地址
			rd_fram_addr = (unsigned int) (frame_buffer_addr[1]
			...
			XAxiVdma_StopParking(&vdma[1], XAXIVDMA_READ);
